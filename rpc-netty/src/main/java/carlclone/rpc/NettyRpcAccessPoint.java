 ...
 */
package carlclone.rpc;

import carlclone.rpc.client.StubFactory;
import carlclone.rpc.server.ServiceProviderRegistry;
import carlclone.rpc.spi.ServiceSupport;
import carlclone.rpc.transport.RequestHandlerRegistry;
import carlclone.rpc.transport.Transport;
import carlclone.rpc.transport.TransportClient;
import carlclone.rpc.transport.TransportServer;
...
public class NettyRpcAccessPoint implements RpcAccessPoint {
    ...
    public synchronized Closeable startServer() throws Exception {
        if (null == server) {
            server = ServiceSupport.load(TransportServer.class);        //加载NettyServer
            server.start(
                    RequestHandlerRegistry.getInstance()        //请求分发注册中心?
                    , port); //端口9999

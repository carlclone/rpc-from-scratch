 ...
 */
package carlclone.rpc.serialize;
...
public class SerializeSupport {
    ...
    private static Map<Class<?>/*序列化对象类型*/, Serializer<?>/*序列化实现*/> serializerMap = new HashMap<>();    // 用于序列化的时候获取实现类
    private static Map<Byte/*序列化实现类型*/, Class<?>/*序列化对象类型*/> typeMap = new HashMap<>();      // 用于反序列化的时候获取实现类

    static {
        for (Serializer serializer : ServiceSupport.loadAll(Serializer.class)) {    //加载所有实现类
